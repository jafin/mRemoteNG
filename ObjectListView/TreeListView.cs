...
// ReSharper restore DoNotCallOverridableMethodsInConstructor
    ...
    {
        get { return base.CheckedObjects; }
        set
        {
            var objectsToRecalculate = new ArrayList(this.CheckedObjects);
            if (value != null)
    ...
    public override IEnumerable ObjectsForClustering
    {
        get
        {
            for (var i = 0; i < this.TreeModel.GetObjectCount(); i++)
...
#pragma warning restore CA2245
    ...
    protected virtual void EnsureTreeRendererPresent(TreeRenderer renderer)
    {
        ...
        var columnZero = this.GetColumn(0);
    ...
    {
        get { return field ??= new TreeRenderer(); }
        set
        {
            field = value ?? new TreeRenderer();
            EnsureTreeRendererPresent(field);
        }
    }
    ...
    {
        var br = this.TreeModel.GetBranch(model);
    ...
    public virtual void Collapse(Object model)
    {
        ...
        var item = this.ModelToItem(model);
        ...
        var index = this.TreeModel.Collapse(model);
        if (index >= 0)
        {
            this.UpdateVirtualListSize();
    ...
    public virtual void CollapseAll()
    {
        ...
        var selection = this.SelectedObjects;
        var index = this.TreeModel.CollapseAll();
        if (index >= 0)
        {
            this.UpdateVirtualListSize();
            this.SelectedObjects = selection;
    ...
    public virtual void Expand(Object model)
    {
        ...
        var index = this.TreeModel.Expand(model);
        if (index < 0)
            return;
        ...
        if (this.RevealAfterExpand && index > 0)
        {
            ...
            try
            {
                ...
                var descedentCount = this.TreeModel.GetVisibleDescendentCount(model);
    ...
    public virtual void ExpandAll()
    {
        ...
        var selection = this.SelectedObjects;
        var index = this.TreeModel.ExpandAll();
        if (index < 0)
            return;
    ...
    {
        var previousTopItemIndex = preserveState ? this.TopItemIndex : -1;
    ...
    protected virtual void RebuildAll(IList selected, IEnumerable expanded, IList checkedObjects)
    {
        ...
        var canExpand = this.CanExpandGetter;
        var childrenGetter = this.ChildrenGetter;
    ...
    public virtual void Reveal(object modelToReveal, bool selectAfterReveal)
    {
        ...
        foreach (var ancestor in this.GetAncestors(modelToReveal))
    ...
    public override void RefreshObjects(IList modelObjects)
    {
        ...
        {
            var newRoots = ObjectListView.EnumerableToArray(this.Roots, false);
            var changed = false;
        ...
        foreach (var model in modelsAndParents.Keys)
        {
            if (model != null)
            {
                var index = this.TreeModel.RebuildChildren(model);
                if (index >= 0)
    ...
    protected override bool SetObjectCheckedness(object modelObject, CheckState state)
    {
        ...
        var checkedness = this.GetCheckState(modelObject);
        if (!checkedness.HasValue || checkedness.Value == CheckState.Indeterminate)
            return true;

        foreach (var child in this.GetChildrenWithoutExpanding(modelObject))
        {
    ...
    public override OLVListItem MakeListViewItem(int itemIndex)
    {
        ...
        var br = this.TreeModel.GetBranch(olvItem.RowObject);
        if (br != null)
    ...
    protected virtual void RecalculateSingleHierarchicalCheckBox(object modelObject)
    {
        ...
        foreach (var child in this.GetChildrenUncached(modelObject))
        {
            var checkedness = this.GetCheckState(child);
    ...
    protected virtual IEnumerable CalculateDistinctAncestors(IList toCheck)
    {
        if (toCheck.Count == 1)
        {
            foreach (var ancestor in this.GetAncestors(toCheck[0]))
            {
        ...
        else
        {
            ...
            {
                foreach (var ancestor in this.GetAncestors(child))
                {
    ...
    {
        var parentGetterDelegate = this.ParentGetter ?? this.GetParent;
    ...
    {
        var focused = this.FocusedItem as OLVListItem;
        if (focused == null)
        {
            base.OnKeyDown(e);
            return;
        ...
        var br = this.TreeModel.GetBranch(modelObject);
    ...
    public class Tree : IVirtualListDataSource, IFilterableDataSource
    {
        ...
        public IEnumerable RootObjects
        {
            ...
            set
            {
                ...
                foreach (var br in this.trunk.ChildBranches)
                    br.RefreshChildren();
                this.RebuildList();
            }
        }
        ...
        public virtual int Collapse(Object model)
        {
            ...
            var index = this.GetObjectIndex(model);
        ...
        protected virtual void RebuildList()
        {
            ...
            var filtered = this.trunk.FilteredChildBranches;
            if (filtered.Count > 0)
            {
        ...
        protected virtual void RebuildObjectMap(int startIndex)
        {
            ...
            for (var i = startIndex; i < this.objectList.Count; i++)
        ...
        public virtual void Sort(OLVColumn column, SortOrder order)
        {
            ...
            foreach (var b in this.trunk.ChildBranches)
        ...
        {
            var newRoots = ObjectListView.EnumerableToArray(this.treeView.Roots, true);
        ...
        public virtual void RemoveObjects(ICollection modelObjects)
        {
            ...
            foreach (var x in this.treeView.Roots)
                newRoots.Add(x);
            foreach (var x in modelObjects)
            {
                newRoots.Remove(x);
        ...
        {
            var newRoots = ObjectListView.EnumerableToArray(this.treeView.Roots, false);
    ...
    public class Branch
    {
        ...
        public virtual IEnumerable Children
        {
            get
            {
                ...
                foreach (var x in this.ChildBranches)
            ...
            set
            {
                ...
                var treeListView = this.Tree.TreeView;
        ...
        {
            var br = this.Tree.GetBranch(childModel);
            if (br == null)
        ...
        public IList<Branch> FilteredChildBranches
        {
            get
            {
                ...
                foreach (var b in this.ChildBranches)
                {
        ...
        public virtual int NumberVisibleDescendents
        {
            get
            {
                ...
                var filtered = this.FilteredChildBranches;
        ...
        {
            this.Expand();
            foreach (var br in this.ChildBranches)
            {
                if (br.CanExpand)
                    br.ExpandAll();
            }
        ...
        public virtual void FlattenOnto(IList flatList)
        {
            ...
            foreach (var br in this.FilteredChildBranches)
            {
                lastBranch = br;
