...
- [ ] 3.1 Write a `KdfPrf` root attribute in `XmlRootNodeSerializer` beside `KdfIterations` (`XmlRootNodeSerializer.cs:22`).
...
- [ ] 5.4 Manual: open a connection file written by the current release, confirm connections decrypt, save, confirm the attribute appears and the file reopens.
- [ ] 5.5 Manual: set a master password on a migrated file, close, reopen, confirm it is still accepted.
- [ ] 5.6 Manual: measure file-open time before and after on a file with 200 connections. The PRF change should not move it; a regression here means the cache keys are wrong and the KDF is running per field.
