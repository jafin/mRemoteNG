...
- [ ] 5.8 Tests: a file outside the profile is written with the recovery protector alone and reports no machine-protector failure when another account opens it; a file inside the profile still gets both; the session cache is not consulted after a lock; a machine protector wrapping a key that is not this file's is rejected rather than accepted.
