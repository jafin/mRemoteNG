...
- [ ] 4.4 Release note for existing users (the one packaging item left): a portable installation that upgrades by copying only the executable, rather than extracting the zip, will come up as the installed edition and appear to have lost its settings. The zip is the supported upgrade path and the marker is why.
