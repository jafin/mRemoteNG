...
for it. The pseudo-random function SHALL default to HMAC-SHA1 when not supplied — see the deviation
recorded against task 1.1: the two arguments fail in opposite directions, and a forgotten function
must produce a file upstream mRemoteNG can still open.
...
- **AND** a pseudo-random function that is not supplied defaults to HMAC-SHA1
