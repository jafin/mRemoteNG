...
- [x] 4.4 Manual, one per shared caller, since all of them replay these diagnostics: a native SSH terminal connection with no key configured reports nothing at error severity; an SFTP connection is unaffected; a file transfer is unaffected. All three confirmed (2026-08-10) — **terminal**: discovery found `~/.ssh/id_rsa`, the connection succeeded, no error raised. **SFTP**: file manager opened from the connection tree, listed the remote directory, no error raised. **File transfer**: Tools → SSH File Transfer, no error raised.
