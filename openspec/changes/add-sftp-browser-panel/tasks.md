...
- [x] 7.7 Note the split host from 1.1 is currently unused by this layout, and either find it a use or remove it. **Removed 2026-08-09** at the maintainer's direction. The dual-pane layout opens as its own tab, so nothing ever called `ShowSidePanel`, and unused UI infrastructure decays. Restorable from `127570bac` if a panel beside a terminal is ever built — and the spike's finding stands recorded in design.md D4 either way, so the next attempt does not have to rediscover that the split belongs *around* `InterfaceControl` rather than inside it.
  - The two defects the spike exposed are independent and were kept: `InterfaceControl.FindInterfaceControl` still searches descendants instead of indexing `Controls[0]`/`[1]` at a fixed depth, and the reconnect test's double still defers to `ProtocolBase.Initialize` instead of copying its tagging logic.
  - `ConnectionTab.OwnerOf` was also kept. It answers correctly at any nesting depth, which the `Parent is ConnectionTab` test it replaced did not.
