...
How an argument on the command line is read: what makes it a switch, where its value begins, and
what happens when the value cannot be used.

Nothing described this before, and two components had drifted into disagreeing about it — one
supporting a form the other could not parse — which is how `--cons C:\path\confCons.xml` came to be
silently ignored rather than either honoured or rejected.

The rules are written once here because both readers are held to them: the interpreter every switch
is read through, and the normalizer that forwards a command line to an already-running instance. A
form that parses in one and not the other is the defect this capability exists to prevent.
