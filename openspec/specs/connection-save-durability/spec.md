# connection-save-durability Specification

## Purpose

What the application guarantees about a change the user has made reaching disk, and what it owes
them when one does not. No change owned this property before, which is how several mechanisms came
to disagree about it: an edit could be discarded on exit, or fail to be written at all, while the
interface behaved as though it had been stored.
