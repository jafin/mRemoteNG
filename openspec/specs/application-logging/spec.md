...
mRemoteNG's file-based diagnostic logging pipeline — what gets logged, at what levels, to what
location, with what rotation policy, and how the log destination can be reconfigured at runtime.
The engine is Serilog (`Serilog.Sinks.File`), replacing log4net; the log file is what a user is
asked to send when reporting a fault, so its contents, its retention and its location are the
behaviour this capability protects.
