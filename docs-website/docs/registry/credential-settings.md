...
If **AllowSaveUsernames** is set to `false`, stored user names in the connection persist until the connection goes through modification or usage. Subsequently, stored user names are removed. Additionally, new connections will not be able to store usernames.
...
If **AllowSavePasswords** is set to `false`, stored passwords in the connection persist until the connection goes through modification or usage. Subsequently, stored passwords are removed. Additionally, new connections will not be able to store passwords.
...
Controls whether the default password field is enabled or disabled. Locking the field may make more sense than disabling the entire settings option.
...
Controls whether the default user via API field is enabled or disabled. Locking the field may make more sense than disabling the entire settings option.
