...
%GUID%  
The unique, stable identifier of the connection. Unlike the name or hostname it never changes, so it is the right token to pass to a script that has to correlate a launch with a specific connection entry.
...
- [External Tools](user-interface/external-tools.md) - For information on using variables with External Tools
