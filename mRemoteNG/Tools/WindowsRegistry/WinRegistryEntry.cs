...
public class WinRegistryEntry<T>
{
    ...
    private RegistryHive _privateHive = RegistryHive.CurrentUser;
    ...
    public T Value
    {
        ...
        set
        {
            _privateValue = ValueValidationRules(value);
        }
    }
    private T _privateValue = default!;
    ...
    private int? _minInt32Value;
    private int? _maxInt32Value;
    private long? _minInt64Value;
    private long? _maxInt64Value;
    ...
    public WinRegistryEntry<T> SetValidation(T[] allowedValues)
    {
        ...
        if (_elementType == typeof(string) && _privateValue != null)
            _privateValue = EnforceStringInputValidity(_privateValue);
    ...
    public WinRegistryEntry<T> SetValidation(int minValue, int maxValue)
    {
        ...
        _minInt32Value = minValue;
        _maxInt32Value = maxValue;
    ...
    public WinRegistryEntry<T> SetValidation(long minValue, long maxValue)
    {
        ...
        _minInt64Value = minValue;
        _maxInt64Value = maxValue;
    ...
    public WinRegistryEntry<T> SetValidation(string minValue, string maxValue)
    {
        ...
        if ((string.IsNullOrEmpty(maxValue) || string.Equals(maxValue, "*", StringComparison.Ordinal)))
            maxValue = (_elementType == typeof(int))
        ...
        else if (_elementType == typeof(long))
        {
    ...
    public WinRegistryEntry<T> Read()
    {
        ...
        try
        {
            ...
            if (key != null)
            {
                _rawValue = rawValue = key.GetValue(name)?.ToString();
    ...
    private T ConvertValueBasedOnType(string originalValue)
    {
        ...
        {
            throw new InvalidOperationException($"Conversion for '{_elementType}' failed.");
        }
        throw new InvalidOperationException($"Conversion not supported for type '{_elementType}'.");
    }
    ...
    private T ValueValidationRules(T value)
    {
        ...
        else if (_elementType == typeof(int) || _elementType == typeof(long))
        {
        ...
        else if (_elementType == typeof(byte))
        {
        ...
        throw new ArgumentException($"Value type '{_elementType.FullName}' is not supported.", nameof(value));
    }
    ...
    private T EnforceStringInputValidity(T value)
    {
        ...
        {
            T? matchedValue = _allowedValues.FirstOrDefault(v => v?.ToString()?.Equals(value?.ToString(), StringComparison.OrdinalIgnoreCase) == true);

            if (matchedValue != null)
                // Correct the Value to ensure the correct spelling and avoid user typing mistakes
                return matchedValue;
        }
        else if (_enumType != null && _enumType.IsEnum)
        {
            var matchedEnumValue = Enum.GetValues(_enumType)
                .Cast<Enum>()
    ...
    private bool ValueKindValidationRule(RegistryValueKind valueKind)
    {
        ...
        return Type.GetTypeCode(_elementType) switch
        {
            ...
            _ => throw new ArgumentException($"Value type '{_elementType.FullName}' is not supported.", nameof(valueKind))
        };
    }
    ...
    /// </summary>
    private bool IsEntrySet() => _rawValue != null && _readOperationSucceeded;
    ...
    private bool CheckIsValid()
    {
        ...
        return Type.GetTypeCode(_elementType) switch
        {
            ...
            _ => throw new ArgumentException($"Value type '{_elementType.FullName}' is not supported.", nameof(_elementType)),
        };
    }
    ...
    private bool ValidateInt32()
    {
        ...
        else if (_minInt32Value != null && _maxInt32Value != null)
            return value >= _minInt32Value && value <= _maxInt32Value;
    ...
    private bool ValidateInt64()
    {
        ...
        else if (_minInt64Value != null && _maxInt64Value != null)
            return value >= _minInt64Value && value <= _maxInt64Value;
