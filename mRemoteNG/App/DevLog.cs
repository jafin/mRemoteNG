...
internal static class DevLog
{
    ...
    internal static void Initialize()
    {
        ...
        var exeDir = Path.GetDirectoryName(System.Windows.Forms.Application.ExecutablePath) ?? ".";
        var markerPath = Path.Combine(exeDir, "verbose.log.enable");
        ...
        try
        {
            File.WriteAllText(_logPath,
                $"=== mRemoteNG Verbose Log — {DateTime.Now:yyyy-MM-dd HH:mm:ss} ==={Environment.NewLine}");
        }
        catch
        {
            _enabled = false;
    ...
    {
        var line = $"{DateTime.Now:HH:mm:ss.fff} [{Environment.CurrentManagedThreadId,3}] {caller}: {message}";
        lock (Lock)
        {
            try
            {
