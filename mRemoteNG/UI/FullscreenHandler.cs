...
public class FullscreenHandler(Form handledForm)
{
    ...
    {
        _savedBorderStyle = handledForm.FormBorderStyle;
        _savedWindowState = handledForm.WindowState;
        _savedBounds = handledForm.Bounds;

        handledForm.FormBorderStyle = FormBorderStyle.None;
        if (handledForm.WindowState == FormWindowState.Maximized)
        {
            handledForm.WindowState = FormWindowState.Normal;
        }

        handledForm.WindowState = FormWindowState.Maximized;
    ...
    private void ExitFullscreen()
    {
        ...
        handledForm.FormBorderStyle = _savedBorderStyle;
        handledForm.WindowState = _savedWindowState;
    ...
    public void MoveToNextScreen()
    {
        ...
        var current = Screen.FromControl(handledForm);
        var idx = Array.IndexOf(screens, current);
    ...
    public void MoveToPreviousScreen()
    {
        ...
        var current = Screen.FromControl(handledForm);
        var idx = Array.IndexOf(screens, current);
        var prev = (idx - 1 + screens.Length) % screens.Length;
    ...
    private void MoveToScreen(Screen target)
    {
        ...
        handledForm.WindowState = FormWindowState.Normal;
        handledForm.Bounds = target.Bounds;
        handledForm.WindowState = FormWindowState.Maximized;
    ...
    private void CheckMousePosition(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            var screen = Screen.FromControl(handledForm);
            ...
            var isOnScreen = screen.Bounds.Contains(cursor);
            var isNearTop = isOnScreen && (cursor.Y < screen.Bounds.Top + 5);
            var isOverToolbar = _toolbar != null && !_toolbar.IsDisposed && _toolbar.Visible &&
                                _toolbar.Bounds.Contains(cursor);

            if (isNearTop || isOverToolbar)
            {
                if (_toolbar == null || _toolbar.IsDisposed)
                {
                    _toolbar = new FullscreenToolbar(handledForm, this);
