...
public class ConnectionTreeSearchTextFilter : IModelFilter
{
    ...
    private bool NodeMatchesFolderNameFilter(ConnectionInfo node)
    {
        ...
        var terms = filterTextLower.Split([' '], StringSplitOptions.RemoveEmptyEntries);
        if (terms.Length == 0)
            return true;

        var nameLower = node.Name.ToLowerInvariant();
        return terms.All(term => nameLower.Contains(term, StringComparison.OrdinalIgnoreCase));
    }
    ...
    private bool NodeMatchesFilter(ConnectionInfo node)
    {
        ...
        if (filterTextLower.StartsWith("protocol:", StringComparison.Ordinal))
        {
            var protocolFilter = filterTextLower[9..].Trim();
        ...
        {
            var tagFilter = filterTextLower[4..].Trim();
        ...
        var nameLower = node.Name.ToLowerInvariant();
        var hostnameLower = node.Hostname.ToLowerInvariant();
        var descriptionLower = node.Description.ToLowerInvariant();
        var tagsLower = (node.EnvironmentTags).ToLowerInvariant();

        return terms.All(term =>
            nameLower.Contains(term, StringComparison.OrdinalIgnoreCase) ||
            hostnameLower.Contains(term, StringComparison.OrdinalIgnoreCase) ||
            descriptionLower.Contains(term, StringComparison.OrdinalIgnoreCase) ||
            tagsLower.Contains(term, StringComparison.OrdinalIgnoreCase));
    }
}