...
### Security
- **Hardened connection-file storage format** — a connection file with no master password was encrypted with a fixed key published in mRemoteNG's own source code, so anyone holding a copy of the file could read every password in it. A file can now be given a random key of its own, wrapped by two independent protectors: your Windows account (nothing to type in daily use) and a **recovery password** for anywhere that account cannot help — another machine, another account, a rebuilt profile, a restored backup. Offered once when a classic file is opened, and available any time from **File → Storage Format...**; declining leaves the file exactly as it was. Hardened files cannot be opened by upstream mRemoteNG or earlier builds, so the dialog offers a classic export first. A file outside your user profile, or one opened by the portable edition, is protected by the recovery password alone — the automatic unlock serves one Windows account and would prompt everyone else on every open. The legacy key becomes read-only: existing files keep opening as they always did, and nothing is ever written under it again. See [Protecting Your Connection File](https://robertpopa22.github.io/mRemoteNG/connection-file-protection)
