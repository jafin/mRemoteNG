...
### Security
- **Hardened connection-file storage format** — a connection file with no master password was encrypted with a fixed key published in mRemoteNG's own source code, so anyone holding a copy of the file could read every password in it. A file can now be given a random key of its own, wrapped by two independent protectors: your Windows account (nothing to type in daily use) and a **recovery password** for anywhere that account cannot help — another machine, another account, a rebuilt profile, a restored backup. Offered once when a classic file is opened, and available any time from **File → Storage Format...**; declining leaves the file exactly as it was. Hardened files cannot be opened by upstream mRemoteNG or earlier builds, so the dialog offers a classic export first. A file opened by the portable edition is protected by the recovery password alone, since there is no account worth binding to on a machine the application was carried to. The legacy key becomes read-only: existing files keep opening as they always did, and nothing is ever written under it again. See [Protecting Your Connection File](https://robertpopa22.github.io/mRemoteNG/connection-file-protection)
- **A shared connection file now unlocks automatically for everyone who uses it.** Previously a hardened file kept where several people could open it was protected by the recovery password alone, so everyone typed it on every open, for ever — which is why the previous release wrote no automatic unlock for a file outside your user profile at all. The file now carries one unlock per person: the first time a colleague opens it they are asked for the recovery password once, and the next time they **save**, their own unlock is added alongside everyone else's. Nobody's unlock replaces anybody else's. Someone who only ever reads the file never earns one, because a file that was only read is not rewritten
- **Removing someone's access — File → Rekey Connection File...** — a colleague who leaves still knows the recovery password and may hold a copy of the file, so deleting their unlock would revoke nothing while appearing to. Rekeying is the operation that does: a new key, a new recovery password, every unlock dropped and yours written, and a backup taken first. The confirmation says what it cannot do — a copy taken beforehand still opens, and every password it held should be treated as known and changed on the systems themselves. Everyone who stays is asked for the new password once, then unlocked again on their next save. See [Removing someone's access](https://robertpopa22.github.io/mRemoteNG/connection-file-protection#removing-someones-access)
- **A save from before a rekey is refused** rather than silently undoing it. A copy of mRemoteNG that had the file open before the rekey still holds the old key and the old recovery password, and its next ordinary save would write the whole file back under them — restoring access for the person the rekey removed, with nobody aware of it. That save now stops with an explanation; nothing is written and the unsaved changes are still on screen. Ordinary saves are untouched and remain last-writer-wins
- **SQL databases can now store their passwords with authenticated encryption.** Every password in a SQL backend was encrypted with a key derived from the master password in a single unsalted step — cheap to attack offline for anyone who could read the tables — and with no way to tell whether the stored text had been altered by someone with write access to the database. New databases now use **AES-256-GCM**, which fixes both: guessing the master password is orders of magnitude more expensive, and a password tampered with in the database is reported as a failure instead of being handed to a connection. Existing databases keep working exactly as they did and stay fully readable and writable; the first save of each session notes in the notification panel that the database is still on the old format. Upgrading one is a deliberate command — **Tools → Options → SQL Server → Upgrade Encryption...**, which appears only when there is something to upgrade — never a prompt when a database is opened, because a database is shared and the decision is not whoever starts mRemoteNG first's to make. The confirmation says what it costs: upstream mRemoteNG and every earlier build stop being able to open the database, that is decided for colleagues who never installed this fork, and it cannot be undone from inside mRemoteNG. The re-encryption runs in one transaction, so a database is never left half-converted, and the master password is checked against the database before a single row is touched. A database newer than the build in use is now refused with both version numbers named. Without that, an older build asks for a master password the database does not have and then reports only that connections could not be loaded, naming nothing. See [SQL Configuration](https://robertpopa22.github.io/mRemoteNG/sql-configuration#upgrading-an-existing-database)
...
### Changed
- **Portable edition is now decided at runtime**, by a `portable.flag` file beside `mRemoteNG.exe`, instead of by how the application was compiled. Previously every published download — including the MSI installer — was built as the portable edition and kept its settings beside the executable. Now the **Self-Contained ZIP is portable** (it ships the marker) while the **framework-dependent ZIP and the MSI are the installed edition**, storing settings and connections in `%APPDATA%\mRemoteNG`. One build serves both editions: create or delete the marker and restart.
  - **Upgrading a portable copy:** extract the whole ZIP. Copying only `mRemoteNG.exe` over an existing folder leaves the new copy without the marker, so it starts as the installed edition and appears to have lost every connection and setting — nothing is deleted, and creating an empty `portable.flag` beside the executable restores it. Nightly users on the framework-dependent ZIP need that file to keep settings beside the executable.
  - The startup log and debug report named the framework's `user.config` for the installed edition, which nothing writes; they now name the settings file that is actually read and written
