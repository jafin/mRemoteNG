...
namespace mRemoteNGTests.FileTransfer
{
    ...
    public class DirectoryTransferExpanderTests
    {
        ...
        public async Task EveryFileBeneathADirectoryIsQueued()
        {
            ...
            Assert.That(items.Select(i => i.SourcePath),
                        Is.EquivalentTo(new[] { "/src/tree/one.txt", "/src/tree/two.txt" }));
        }
        ...
        public async Task ADirectoryThatCannotBeCreatedAbandonsOnlyThatBranch()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(items.Select(i => i.SourcePath), Is.EqualTo(new[] { "/src/tree/fine/reachable.txt" }));
        ...
        public async Task ADirectoryThatCannotBeListedLeavesSiblingsAlone()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(items.Select(i => i.SourcePath), Is.EqualTo(new[] { "/src/tree/open/readable.txt" }));
                Assert.That(skipped, Has.Count.EqualTo(1));
            });
        }
        ...
        public async Task ALinkCycleTerminates()
        {
            ...
            Assert.That(items.Select(i => i.SourcePath), Is.EqualTo(new[] { "/src/tree/real.txt" }));
        }
        ...
        public async Task TheDepthLimitStopsDescentAndIsReported()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(items.Select(i => i.SourcePath), Is.EqualTo(new[] { "/src/tree/shallow.txt" }));
        ...
        public async Task SkipAllQueuesNothingThatExists()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(items.Select(i => i.SourcePath), Is.EqualTo(new[] { "/src/tree/fresh.txt" }));
        ...
        public async Task OverwriteIfNewerQueuesOnlyTheNewerSource()
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(items.Select(i => i.SourcePath), Is.EqualTo(new[] { "/src/tree/newer.txt" }));
                Assert.That(expander.SkippedExistingCount, Is.EqualTo(1));
            });
        }
